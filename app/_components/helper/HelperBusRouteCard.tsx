...
interface Props {
  ...
  /** When loaded from the API roster, prefer this count over `route.studentsCount`. */
  liveStudentCount?: number;
}

export default function HelperBusRouteCard({ assignment, liveStudentCount }: Props) {
  const { bus, route } = assignment;
  const studentCount =
    typeof liveStudentCount === "number" && liveStudentCount >= 0
      ? liveStudentCount
      : route.studentsCount;
